...
- диагностика сети, сканирование локальных подсетей и временный доступ к подсети сетевого устройства без отключения DHCP.

### Временный доступ к сетевому устройству

Функция помогает подключиться к принтеру, кассе или другому устройству, которое находится в другой IPv4-подсети, не меняя основную сетевую конфигурацию компьютера. Это удобно, например, когда адрес устройства известен, но открыть его веб-интерфейс или проверить доступность с текущими настройками сети нельзя.

1. Откройте **Утилиты обслуживания → Диагностика сети → Временный доступ к подсети устройства**.
2. Введите текущий IPv4-адрес устройства.
3. Если на компьютере несколько сетевых адаптеров, выберите тот, к которому подключено устройство.
4. Проверьте предложенный временный адрес и подтвердите операцию.
5. Работайте с устройством из открывшегося меню: можно повторно проверить доступность, открыть его веб-интерфейс или продлить сеанс.
6. Чтобы проверить подсеть встроенным сканером, выберите **Оставить работать в фоне**, запустите сканирование и затем вернитесь в этот экран.
7. По окончании выберите **Завершить временный доступ**.

goMH добавляет компьютеру отдельный временный IPv4-адрес на 30 минут. DHCP, основной IP-адрес, шлюз и DNS при этом не изменяются. Выход из экрана оставляет адрес работать в фоне; при повторном входе можно вернуться к активному сеансу. Созданный адрес удаляется при явном завершении сеанса или автоматически по таймеру; защитная задача очистки срабатывает и после аварийного закрытия программы. Если устройство уже доступно, программа сообщит, что временный адрес не требуется.

Функция работает с устройствами в подсети `/24` и требует запуска goMH от имени администратора. Перед выбором адаптера убедитесь, что это физическое подключение к нужной сети, особенно если на компьютере активны Wi-Fi, VPN или виртуальные адаптеры.
